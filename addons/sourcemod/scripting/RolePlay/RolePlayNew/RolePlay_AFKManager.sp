...
#define CONVAR_BUTTONSBUFFER							13
#define CONVAR_ARRAY_SIZE								14
...
Obs_Mode iObserverMode[MAXPLAYERS + 1]; 							// Observer Mode
...
Action Forward_OnAFKEvent(Handle plugin, const char[] name, int client) // forward Action AFKM_OnAFKEvent(const char[] name, int client);
{
	...
	if (func != INVALID_FUNCTION)
	{
		if (AddToForward(g_FWD_hOnAFKEvent, plugin, func))
		{
			...
			Call_PushCell(client);
			Call_Finish(result);
			ResAllFromForward(g_FWD_hOnAFKEvent, plugin);
		}
	} else
...
#endif
	iObserverMode[index] = OBS_MODE_NONE;
...
public void RP_OnEcoLoaded()
{
	...
	GetTrieString(Trie_JailEco, "AFK_XYZ", TempFormat, sizeof(TempFormat));
	
	StringToVector(TempFormat, AFK_XYZ);
...
public void OnPluginStart() // AFK Manager Plugin has started
{
	...
	if (strcmp(game_mod, "csgo", false) == 0)
	{
		...
		if (g_hAFKTimer[client] != INVALID_HANDLE)
		{
			if (g_iButtonsArraySize > 0)
			{
				...
				if (iLastButtons != buttons) // ( (angles[0] != fEyeAngles[client][0]) || (angles[1] != fEyeAngles[client][1]) || (angles[2] != fEyeAngles[client][2]) )
				{
					...
					{
						if (iObserverMode[client] == OBS_MODE_NONE) // Player has an Invalid Observer Mode
						{
							iButtonsArray[client][iButtonsArrayIndex[client]] = buttons;
							//fEyeAngles[client] = angles;
							return Plugin_Continue;
						}
						else if (iObserverMode[client] != OBS_MODE_IN_EYE) // Check Observer Mode in case it has changed
							iObserverMode[client] = view_as<Obs_Mode>(GetEntProp(client, Prop_Send, "m_iObserverMode"));

						if ((iObserverMode[client] == OBS_MODE_IN_EYE) && (iLastButtons == buttons))
						{
#if _DEBUG > 1
...
public Action OnClientCommand(int client, int args) // Player Chat
{
	if (g_bEnabled)
	{	
		ResetPlayer(client, false); // Reset timer once player has said something in chat.
...
public Action Timer_CheckPlayer(Handle Timer, int client) // General AFK Timers
{
	...
	if(g_bEnabled) // Is the AFK Manager Enabled
	{
		...
		if (IsClientObserver(client))
		{
			Obs_Mode m_iObserverMode = view_as<Obs_Mode>(GetEntProp(client, Prop_Send, "m_iObserverMode"));
#if _DEBUG > 1
			LogDebug(false, "Timer_CheckPlayer - Client: %i is Observer Current Mode: %i", client, m_iObserverMode);
#endif
			if (iObserverMode[client] == OBS_MODE_NONE) // Invalid Observer Mode
			{
#if _DEBUG > 1
...
Action MoveAFKClient(int client, bool Advertise=true) // Move AFK Client to Spectator Team
{
	...
	isSpec[client] = true;
	g_iAFKTime[client] = -86400;
	bPlayerAFK[client] = true;
	
	RP_SetClientJob(client, -1);
	
	TeleportEntity(client, AFK_XYZ, NULL_VECTOR, NULL_VECTOR);
